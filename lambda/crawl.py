...
def fetch_url(url, pages):
    ...
    for script in soup.find_all('script'):
        if script.get('src') is not None and not isAbsoluteUrl(script.get('src')):
    ...
    for stylesheet in soup.find_all('link[rel="stylesheet"]'):
        if stylesheet.get('href') is not None and not isAbsoluteUrl(stylesheet.get('href')):
            print stylesheet.get('href')
            stylesheet.href = urljoin(prefix, stylesheet.href)
            print stylesheet.get('href')

    for img in soup.find_all('img'):
        if img.get('src') is not None and not isAbsoluteUrl(img.get('src')):
            print img.get('src')
